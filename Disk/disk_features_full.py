...
def parse_iostat(file_path):
    ...
    # Filter only real disk (avoid loop devices)
    df = df[df["Device"] == "sda"]
...
def compute_features(iostat_df, vmstat_df, pidstat_data, psi_data):
    ...
    elif "r_await" in iostat_df.columns and "w_await" in iostat_df.columns:
        features["avg_await"] = (
            iostat_df["r_await"].fillna(0) +
            iostat_df["w_await"].fillna(0)
        ).mean() / 2
    ...
    # -----------------------------
    # ⭐ NEW: Request Size (CRITICAL)
    # -----------------------------
    if "rareq-sz" in iostat_df.columns:
        features["avg_req_size"] = iostat_df["rareq-sz"].mean()
    elif "wareq-sz" in iostat_df.columns:
        features["avg_req_size"] = iostat_df["wareq-sz"].mean()
    else:
        features["avg_req_size"] = 0

    # -----------------------------
    # ⭐ NEW: Write Ratio (for mixed workload detection)
    # -----------------------------
    if "r/s" in iostat_df.columns and "w/s" in iostat_df.columns:
        total_io = iostat_df["r/s"] + iostat_df["w/s"]
        # Avoid division by zero; compute per-sample then average
        write_frac = iostat_df["w/s"] / total_io.replace(0, float("nan"))
        features["write_ratio"] = write_frac.mean()
    else:
        features["write_ratio"] = 0.0
