 ...
 *
 * Each thread independently computes all primes up to UPPER_LIMIT.
 * Deterministic: same input always produces same output and runtime.
 * CPU-bound: no I/O, no sleep, no syscalls during computation.
 */
...
static int is_prime(unsigned long n)
{
    ...
    if (n == 2)      return 1;
    if (n % 2 == 0)  return 0;
    unsigned long lim = (unsigned long)sqrt((double)n);
...
int main(int argc, char *argv[])
{
    ...
    if (num_threads < 1 || num_threads > 64) {
        fprintf(stderr, "Error: num_threads must be 1-64\n");
        return 1;
    }
    ...
    for (int i = 0; i < num_threads; i++) {
        args[i].thread_id = i;
        args[i].count     = 0;
        if (pthread_create(&threads[i], NULL, compute_primes, &args[i])) {
